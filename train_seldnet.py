...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        elif params['lr_scheduler'] == 'CosineAnnealingWarmRestarts':
            scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2, eta_min=0, verbose=True)

        print(' optim lr_scheduler is = ', params['lr_scheduler'])
        ...
        save_model_path = 'D:/tutorials/University/AASP/proposal/dataset/SELD - DCASE/2023/models/saved model and optimizer in spesific epoch'
        ...
        # # حلقه آموزش ادامه می‌یابد
        # pass
        # model.load_state_dict(torch.load('D:/tutorials/University/AASP/proposal/dataset/SELD - DCASE/2023/models/6_1_dev_split0_multiaccdoa_mic_gcc_model.h5', map_location='cpu'))
       ...
       ###################### #  تنظیم منطقه زمانی به وقت تهران ######################################################
        ...
        output_result_prosedure_file = 'D:/tutorials/University/AASP/proposal/dataset/SELD - DCASE/2023/results/Network Results Procedure'
        output_result_prosedure_file = os.path.join(output_result_prosedure_file, '{}_{}'.format(shamsi_date.strftime('%Y''.''%m''.''%d''_''%H''-''%M''-''%S'), 'seld_results.csv' ))
        output_BESTresult_prosedure_file = os.path.join('D:/tutorials/University/AASP/proposal/dataset/SELD - DCASE/2023/results/Network Results Procedure', '{}_{}'.format(shamsi_date.strftime('%Y''.''%m''.''%d''_''%H''-''%M''-''%S'), 'seld_BESTresults.csv' ))
        begin_time = time.time()
        # بارگذاری مدل
        # checkpoint = torch.load('D:/tutorials/University/AASP/proposal/network results/14031121(training on a part of 2022synth and uavmix and bell_... dataset )(all synth and no real data)/best models/1403.11.22_06-07-38_seld_BESTmodel.pth')  
        # model.load_state_dict(checkpoint['model_state_dict'])
        # optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        # start_epoch = checkpoint['epoch']
        # # ادامه آموزش
        # for epoch_cnt in range(start_epoch, nb_epoch):
        # # start_epoch = 100
        for epoch_cnt in range(nb_epoch):
        #     # if epoch_cnt == 143:
        #     #     for param_group in optimizer.param_groups:
        #     #         param_group['lr'] = 0.00031


        #     # کاهش لرنینگ ریت هر 20 ایپاک
        #     # if ( epoch_cnt >= start_epoch):
        #     #     if (epoch_cnt % 5 == 0):
        #     #         for param_group in optimizer.param_groups:
        #     #             param_group['lr'] *= 0.8
        #     # current_lr = optimizer.param_groups[0]['lr']
            ...
            try:
                with open(output_result_prosedure_file, mode='x', newline='', encoding='utf-8') as file:
                    writer = csv.writer(file)
                    # نوشتن هدر
                    writer.writerow(["epoch","learning rate", "train_time", "val_time", "train_loss","val_loss", "ER", "F", "LE", "LR", "SELD_scr"])
            except FileExistsError:
                pass  # اگر فایل وجود دارد، هدر را دوباره ننویسید

            with open(output_result_prosedure_file, mode='a', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                # نوشتن مقادیر در فایل
                writer.writerow([ epoch_cnt, current_lr, train_time, val_time, train_loss, val_loss, val_ER, val_F, val_LE, val_LR, val_seld_scr])
            ...
            if val_seld_scr <= best_seld_scr:
                ...
                # تبدیل تاریخ به شمسی
                shamsi_date = jdatetime.datetime.fromgregorian(datetime=tehran_time)
                output_BESTmodel = os.path.join(save_model_path, '{}_{}'.format(shamsi_date.strftime('%Y''.''%m''.''%d''_''%H''-''%M''-''%S'), 'seld_BESTmodel.pth' ))
                torch.save({
                'epoch': epoch_cnt + 1,
                ...
                try:
                    with open(output_BESTresult_prosedure_file, mode='x', newline='', encoding='utf-8') as file:
                        writer = csv.writer(file)
                        # نوشتن هدر
                        writer.writerow(["best_val_epoch","learning rate", "best_train_loss","best_val_loss", "best_ER", "best_F", "best_LE", "best_LR", "best_seld_scr"])
                except FileExistsError:
                    pass  # اگر فایل وجود دارد، هدر را دوباره ننویسید

                with open(output_BESTresult_prosedure_file, mode='a', newline='', encoding='utf-8') as file:
                    writer = csv.writer(file)
                    # نوشتن مقادیر در فایل
                    writer.writerow([best_val_epoch, current_lr, train_loss, val_loss, best_ER, best_F, best_LE, best_LR, best_seld_scr])
        ...
        end_time = time.time()
        train_time_file = os.path.join('D:/tutorials/University/AASP/proposal/dataset/SELD - DCASE/2023/results/Network Results Procedure', '{}_{}'.format(shamsi_date.strftime('%Y''.''%m''.''%d''_''%H''-''%M''-''%S'), 'trainning_time_model.csv' ))
        try:
            with open(train_time_file, mode='x', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                # نوشتن هدر
                writer.writerow(["train_time(min)"])
        except FileExistsError:
            pass  # اگر فایل وجود دارد، هدر را دوباره ننویسید
        with open(train_time_file, mode='a', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            # نوشتن مقادیر در فایل
            writer.writerow([(end_time - begin_time)//60])
        # ---------------------------------------------------------------------
        # Evaluate on unseen test data
        # ---------------------------------------------------------------------
        # # بارگذاری مدل
        # checkpoint = torch.load('D:/tutorials/University/AASP/proposal/dataset/SELD - DCASE/2023/models/saved model and optimizer in spesific epoch/1403.11.28_15-58-43_seld_BESTmodel.pth', map_location='cpu')
        # model.load_state_dict(checkpoint['model_state_dict'])
        # optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        # print('model loaded')
        ...
        # Dump results in DCASE output format for calculating final scores
        dcase_output_test_folder = os.path.join(params['dcase_output_dir'], '{}_{}_test'.format(unique_name, shamsi_date.strftime('%Y''.''%m''.''%d''_''%H''-''%M''-''%S')))
        ...
        # print(' classwise_test_scr shape = ', np.array(classwise_test_scr).shape)
        print('\nTest Loss')
